...
def EPE(pred, gt):
    """
    Calculate End Point Error (EPE) for homography estimation

    Args:
        pred (torch.Tensor): Predicted homography parameters
        gt (torch.Tensor): Ground truth homography parameters

    Returns:
        torch.Tensor: Average End Point Error
    """
    # Ensure both pred and gt are in the same shape
    pred = pred.view(-1, 8)
    gt = gt.view(-1, 8)

    # Calculate the Euclidean distance between predicted and ground truth points
