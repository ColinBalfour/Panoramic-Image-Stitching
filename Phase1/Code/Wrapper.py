...
def main():
    ...
    #     """
    #     Implements ANMS on Harris corner responses
    #     Args:
    #         corner_img: Harris corner response image
    #         n_best: Number of best corners to keep
    #     Returns:
    #         List of (y,x) coordinates of selected corners
    #     """
    #     # Find all local maxima (these are our strong corners - N_strong)
    #     lm = peak_local_max(corner_img, min_distance=1)
    #     n_strong = len(lm)
    #
    #     if n_strong == 0:
    #         return []
    ...
    #         y_i, x_i = lm[i]  # Note: peak_local_max returns (y,x)
    #
    #         # For each point j
    ...
    #             y_j, x_j = lm[j]
    #
    #             # If corner response at j is greater than at i
    #             if corner_img[y_j, x_j] > corner_img[y_i, x_i]:
    #                 # Calculate Euclidean distance
    #                 ED = (x_j - x_i) ** 2 + (y_j - y_i) ** 2
    #
    #                 # Update r_i if ED is smaller
    #                 if ED < r[i]:
    #                     r[i] = ED
    #
    #     # Sort points based on r values and take top n_best
    #     points_with_r = list(zip(lm, r))
    #     points_with_r.sort(key=lambda x: x[1], reverse=True)
    #
    #     # Return top n_best points
    #     selected_points = [(y, x) for (y, x), _ in points_with_r[:n_best]]
    ...
    #     if not USE_SUBPIX:
    #         features = apply_anms(im, n_best=500)  # Adjust n_best as needed
    #         features_set.append(features)
    #
    #         # Visualization
    #         for y, x in features:
    #             im_set[i][x, y] = [0, 0, 255]
    #             # For better visualization, you might want to use:
    #             # cv2.circle(im_set[i], (x, y), 2, (0, 0, 255), -1)
    #
    ...
    def create_overlap_mask(img1, img2):
        ...
        mask1 = np.zeros((h_max, w_max), dtype=np.float32)
        mask2 = np.zeros((h_max, w_max), dtype=np.float32)

        # Fill masks for each image
        mask1[:img1.shape[0], :img1.shape[1]] = np.any(img1 > 0, axis=2)
        mask2[:img2.shape[0], :img2.shape[1]] = np.any(img2 > 0, axis=2)
    ...
    def blend_images_complete(img1, img2):
        ...
        # Create consistent-sized images
        img1_padded = np.zeros((h_max, w_max, 3), dtype=np.float32)
        img2_padded = np.zeros((h_max, w_max, 3), dtype=np.float32)

        # Copy original images to padded images
        img1_padded[:img1.shape[0], :img1.shape[1]] = img1
        img2_padded[:img2.shape[0], :img2.shape[1]] = img2

        # Create overlap mask
        overlap_mask = create_overlap_mask(img1, img2)

        # Adjust exposure of img1 to match img2 in overlap region
        img1_adjusted = adjust_exposure(img1_padded, img2_padded, overlap_mask)
        ...
        blended = np.zeros_like(img1_padded, dtype=np.float32)

        # Where both images have content, blend using weights
        overlap = (overlap_mask > 0)
        blended[overlap] = (
                img1_adjusted[overlap] * weights1[overlap] +
                img2_padded[overlap] * weights2[overlap]
        )

        # Where only img1 has content
        img1_only = np.logical_and(np.any(img1_padded > 0, axis=2), ~overlap)
        blended[img1_only] = img1_adjusted[img1_only]

        # Where only img2 has content
        img2_only = np.logical_and(np.any(img2_padded > 0, axis=2), ~overlap)
        blended[img2_only] = img2_padded[img2_only]
    ...
    def stitch_panorama_in_parts(im_set):
        total_images = len(im_set)

        if total_images <= 0:
            raise ValueError("No images to stitch")

        if total_images == 1:
            return im_set[0]  # Return the single image

        if total_images == 2:
            matches = match_features(discriptors_set[0], discriptors_set[1])
            matches = np.array(matches)
            ret, H, inliers = RANSAC(features_set[0], features_set[1], matches)

            if not ret:
                return np.concatenate((im_set[0], im_set[1]), axis=1)

            warped_images = [
                cv2.warpPerspective(im_set[0], H,
                                    (im_set[0].shape[1] + im_set[1].shape[1],
                                     max(im_set[0].shape[0], im_set[1].shape[0]))),
                im_set[1]
            ]
            return blend_images_complete(warped_images[0], warped_images[1])

        # For 3 or more images
        if total_images % 2 == 0:
            # Even number of images - split equally
            mid_point = total_images // 2
            first_half = im_set[:mid_point]
            second_half = im_set[mid_point:]
        else:
            # Odd number of images - include one more image in first half
            mid_point = total_images // 2
            first_half = im_set[:mid_point + 1]
            second_half = im_set[mid_point:]

        # Stitch first half
        first_half_features_set = features_set[:len(first_half)]
        first_half_discriptors_set = discriptors_set[:len(first_half)]
        first_half_warped_images = []
        first_half_cumulative_homographies = {0: (0, np.eye(3))}

        # Perform feature matching and homography estimation for first half
        for i in range(len(first_half) - 1):
            matches = match_features(first_half_discriptors_set[i], first_half_discriptors_set[i + 1])
            matches = np.array(matches)

            if len(matches) < 8:
                continue

            ret, H, inliers = RANSAC(first_half_features_set[i], first_half_features_set[i + 1], matches)
            ...
            if i == 0:
                first_half_cumulative_homographies[i + 1] = (0, H)
            else:
                H_cumulative = H @ first_half_cumulative_homographies[i][1]
                first_half_cumulative_homographies[i + 1] = (i, H_cumulative)

        # Warp first half images
        first_half_corners = []
        f = 750
        for i, image in enumerate(first_half):
            from_index, H = first_half_cumulative_homographies[i]
            h, w = image.shape[:2]
            corners = np.array([[0, 0, 1], [w, 0, 1], [w, h, 1], [0, h, 1]]).T
            warped_corners = np.linalg.inv(H) @ corners
            warped_corners = warped_corners / warped_corners[2]
            first_half_corners.append(warped_corners[:2].T)

        # size of canvas
        all_corners = np.vstack(first_half_corners)
        min_x, min_y = np.floor(all_corners.min(axis=0)).astype(int)
        max_x, max_y = np.ceil(all_corners.max(axis=0)).astype(int)
        canvas_width = max_x - min_x
        canvas_height = max_y - min_y
        ...
        for i, image in enumerate(first_half):
            ...
            first_half_warped_images.append(warped)
            # # Equirectangular warping
            # warped = equirectangular_warp(image, focal_length=500)
            #
            # # Apply homography
            ...
            second_half_features_set = features_set[mid_point:]
            second_half_discriptors_set = discriptors_set[mid_point:]
            second_half_warped_images = []
            second_half_cumulative_homographies = {0: (0, np.eye(3))}

            # Perform feature matching and homography estimation for second half
            for i in range(len(second_half) - 1):
                matches = match_features(second_half_discriptors_set[i], second_half_discriptors_set[i + 1])
                matches = np.array(matches)

                if len(matches) < 8:
                    continue

                ret, H, inliers = RANSAC(second_half_features_set[i], second_half_features_set[i + 1], matches)
                ...
                # Update cumulative homographies
                if i == 0:
                    second_half_cumulative_homographies[i + 1] = (0, H)
                else:
                    H_cumulative = H @ second_half_cumulative_homographies[i][1]
                    second_half_cumulative_homographies[i + 1] = (i, H_cumulative)

            # Warp second half images
            second_half_corners = []
            f = 500
            for i, image in enumerate(second_half):
                from_index, H = second_half_cumulative_homographies[i]
                h, w = image.shape[:2]
                corners = np.array([[0, 0, 1], [w, 0, 1], [w, h, 1], [0, h, 1]]).T
                warped_corners = np.linalg.inv(H) @ corners
                warped_corners = warped_corners / warped_corners[2]
                second_half_corners.append(warped_corners[:2].T)

            # size of canvas for second half
            all_corners = np.vstack(second_half_corners)
            min_x, min_y = np.floor(all_corners.min(axis=0)).astype(int)
            max_x, max_y = np.ceil(all_corners.max(axis=0)).astype(int)
            canvas_width = max_x - min_x
            canvas_height = max_y - min_y
            ...
            for i, image in enumerate(second_half):
                ...
                second_half_warped_images.append(warped)

                # Replace the final concatenation part with this:

            def combine_panoramas(first_half_warped, overlap_blended, second_half_warped):
                # Get maximum dimensions
                max_height = max(
                    max(img.shape[0] for img in first_half_warped[:-1] if img is not None),
                    overlap_blended.shape[0] if overlap_blended is not None else 0,
                    max(img.shape[0] for img in second_half_warped[1:] if img is not None)
                )
                total_width = sum(img.shape[1] for img in first_half_warped[:-1] if img is not None) + \
                              (overlap_blended.shape[1] if overlap_blended is not None else 0) + \
                              sum(img.shape[1] for img in second_half_warped[1:] if img is not None)

                # Create canvas
                final = np.zeros((max_height, total_width, 3), dtype=np.uint8)
                current_x = 0

                # Add first half
                for img in first_half_warped[:-1]:
                    if img is not None:
                        h, w = img.shape[:2]
                        # Create mask for non-black pixels
                        mask = (img > 0).any(axis=2).astype(np.uint8) * 255
                        # Create ROI
                        roi = final[:h, current_x:current_x + w]
                        # Add image
                        final[:h, current_x:current_x + w] = cv2.add(roi, img, mask=mask)
                        current_x += w

                # Add overlap region
                if overlap_blended is not None:
                    h, w = overlap_blended.shape[:2]
                    mask = (overlap_blended > 0).any(axis=2).astype(np.uint8) * 255
                    roi = final[:h, current_x:current_x + w]
                    final[:h, current_x:current_x + w] = cv2.add(roi, overlap_blended, mask=mask)
                    current_x += w

                # Add second half
                for img in second_half_warped[1:]:
                    if img is not None:
                        h, w = img.shape[:2]
                        mask = (img > 0).any(axis=2).astype(np.uint8) * 255
                        roi = final[:h, current_x:current_x + w]
                ...
                # Replace the final concatenation with the new combine function
            ...
            # Find the last image of first half and first image of second half
            overlap_images = [
                first_half_warped_images[-1],
                second_half_warped_images[0]
            ]

            # Blend the overlap region
            overlap_blended = blend_images_complete(overlap_images[0], overlap_images[1])

            final_panorama = combine_panoramas(
                first_half_warped_images,
                overlap_blended,
                second_half_warped_images
            )
    ...
    final_result = stitch_panorama_in_parts(im_set)
